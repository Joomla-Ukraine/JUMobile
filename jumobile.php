 ...
 * @copyright        2016-2021 (C) Joomla! Ukraine, https://joomla-ua.org. All rights reserved.
 ...
 * @since  1.0
 */
class plgSystemJUMobile extends CMSPlugin
{
	...
	public function __construct(&$subject, $params)
	{
		...
		$this->_plugin = PluginHelper::getPlugin('system', 'jumobile');
	...
	public function onAfterRender()
	{
		...
		if(($this->devMode == '1' || ($lib_md && $this->isMobile) || ((!$lib_md && $browser->isMobile()) || false !== stripos($agent, 'mobile'))) && $this->params->get('allowcache') == 1 && $enabled)
		{
			$app->allowCache(true);
	...
	public function onAfterInitialise()
	{
		...
		if($this->devMode == '1' || ($lib_md && $this->isMobile) || ((!$lib_md && $browser->isMobile()) || false !== stripos($agent, 'mobile')) // stristr($agent, 'mobile')
		)
		{
			if($enabled)
			{
				...
				if($redirect && (!$app->getUserState('jumobile.isredirected', false) || !$redirectOnce))
				{
					$uri = Uri::getInstance();
					if(substr($uri->current(), 0, strlen($mobiledomain)) != $mobiledomain)
					{
						$app->setUserState('jumobile.isredirected', true);
						$page = ltrim($uri->toString([ 'path', 'query' ]), '/');
